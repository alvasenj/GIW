...
for i in etiquetas:
    if(d>1):
        ...
        tupla = etiqueta.partition("http://trenesytiempos.blogspot.com.es/")
        protocolo, separador, dominio = tupla
        formato = "{0}".format(dominio)
        ...
        j=0 
        os.mkdir(str(formato))#Creamos el directorio para cada entrada
        #Leemos las imágenes de dicha entrada y las guardamos en un .jpg
        for k in etiquetasImagen:
            archivo=open(str(formato)+"/foto"+str(j)+".jpg","wb")
